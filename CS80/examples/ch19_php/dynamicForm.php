...
// isset determines whether or not a variable exists
// each of these variables will only exist after the form has been submitted
...
if ( isset( $_POST["submit"] ) )
{
  ...
  // if there is not an error, connect to the database and insert the content
  if ( !$iserror )
  {
    // build INSERT query
    // mysql_real_escape_string adds appropiate escape '\' where necessary for mysql
...
// HOW THIS WORKS
// ---------------------------------------------
// We make the form self-submitting, meaning when the user fills out data, we reload the page
// This is the most important mechanism of thise form. On the first execution, the "submission" will fail 
// (specifically the $_POST["submit"] has not been set, so we come to this portion of the php immediately 
//  on the first execution)
// When we click submit, we reload *this* page, but the $_POST superarray will have content! which means 
// the validation will execute, and if there are errors, we will "reprint" the HTML page below
...
{
  // $book stores the previous book, we want to select it by default if there's an error
